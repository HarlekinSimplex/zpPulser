...
static char current_namespace[16] = CONFIG_NVS_OPTIONS_NAMESPACE ;
...
{
    list_args.partition->sval[0] = CONFIG_NVS_OPTIONS_PARTITION ;
...
void register_nvs(void)
{
    ...
    list_args.partition = arg_str0(NULL, NULL, "<partition>", "partition name");
    ...
    const esp_console_cmd_t list_entries_cmd = {
        ...
        "Following command list variables stored inside 'nvs' partition, under namespace 'config' with type uint32_t"
        "Example: nvs_list nvs -n config -t u32 \n",
        .hint = NULL,
        .func = &list_entries,
        .argtable = &list_args
    };
...
uint8_t option_get_u8(const char* option_key)
{
    ...
    esp_err = nvs_open(current_namespace, NVS_READONLY, &nvs);
    if (esp_err != ESP_OK) {
		ESP_LOGE(TAG, "%s", esp_err_to_name(esp_err));
        return 0 ;
    }

    // Check specified configuration option
    esp_err = nvs_get_u8(nvs, option_key, &out_value) ;
    if (esp_err != ESP_OK) {
		ESP_LOGE(TAG, "%s", esp_err_to_name(esp_err));
        return 0 ;
    }
...
uint16_t option_get_u16(const char* option_key)
{
    ...
    // Check specified configuration option
    esp_err = nvs_get_u16(nvs, option_key, &out_value) ;
    if (esp_err != ESP_OK) {
		ESP_LOGE(TAG, "%s", esp_err_to_name(esp_err));
        return 0 ;
    }
...
uint32_t option_get_u32(const char* option_key)
{
    ...
    // Check specified configuration option
    esp_err = nvs_get_u32(nvs, option_key, &out_value) ;
    if (esp_err != ESP_OK) {
		ESP_LOGE(TAG, "%s", esp_err_to_name(esp_err));
        return 0 ;
    }
...
float option_get_float(const char* option_key)
{
    ...
    // Get required buffer length for value from flash
    esp_err = nvs_get_str(nvs, option_key, NULL, &string_length) ;
    if (esp_err == ESP_OK && string_length > 0) {
    	// Key exists so alloc buffer and read and covert valuevalue
    	// Buffer allocation
        string_buffer = malloc(string_length) ;
        // Read string from flash
        esp_err = nvs_get_str(nvs, option_key, string_buffer, &string_length) ;
    	ESP_LOGD(TAG, "Value read %s", string_buffer);
        // Convert value to float
    	out_value = atof(string_buffer) ;
        // Free used buffer
    	free(string_buffer) ;
    }
...
esp_err_t initialize_nvs(int force_flag)
{
    ...
    if (err == ESP_ERR_NVS_NO_FREE_PAGES || err == ESP_ERR_NVS_NEW_VERSION_FOUND) {
        ...
        err = nvs_flash_init();
		ESP_LOGI(TAG, "Flash has been erased because of no free pages or new nvs firmware version");
    }
    ESP_ERROR_CHECK(err);
